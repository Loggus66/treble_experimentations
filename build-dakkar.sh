...
download_patches() {
	...
	if [[ $localManifestBranch == android-10.0 ]];then
		...
		githubMatch=v1..
	elif [[ $localManifestBranch == android-8.1 ]];then
		githubMatch=v32
	else
		githubMatch=v..
	fi
    jq --help > /dev/null
	wantedRelease="$(curl --silent https://api.github.com/repos/phhusson/treble_experimentations/releases |jq -r '.[] | .tag_name' |grep -E "$githubMatch\$" || echo $githubMatch |sort -V | tail -n 1)"
